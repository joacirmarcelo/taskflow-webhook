...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/webhook-messages" && req.method === "GET") {
    // Return only unprocessed (for sync logic)
    return json(res, webhookMessages.filter(m => !m.processed));
  }
  if (url === "/webhook-messages/all" && req.method === "GET") {
    // Return all messages (for conversation display)
    return json(res, webhookMessages);
  }
  if (url === "/webhook-messages/count" && req.method === "GET") {
    return json(res, { count: webhookMessages.filter(m => !m.processed).length });
  }
  if (url === "/webhook-messages/clear" && req.method === "POST") {
    // Mark as processed instead of deleting — so messages stay visible
  ...
  if (url === "/webhook-messages/purge" && req.method === "POST") {
    // Actually delete all (manual cleanup)
