...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      if (needsText) {
        // Z-API sends isGroup flag directly; phone = group id for groups
        const chatId = p.chatId || p.phone || p.from || "";
        const isGroup = p.isGroup === true || chatId.includes("@g.us");
        let groupId = null, groupName = null;
        if (isGroup) {
          groupId = String(p.phone || chatId).replace(/@\S+$/, "").replace(/[^0-9-]/g, "");
          // In Z-API, chatName IS the group's name for group messages.
          // senderName is the participant — NEVER use it as the group name.
          const gn = (p.chatName || p.groupName || "").trim();
          groupName = (gn && !/^\d+$/.test(gn)) ? gn : `Grupo ${groupId.slice(-6)}`;
        }
        // Sender phone — for groups this is participantPhone (who sent it)
        const rawSender = (isGroup ? (p.participantPhone || p.author || p.participant || p.senderPhone) : (p.phone || p.from || p.chatId || ""))
          .replace(/@\S+$/, "").replace(/:\d+$/, "");
        ...
        if (fromMe) {
          ...
          // For groups, senderName = the participant who sent (shown above their bubble).
          // For direct chats, senderName = the contact's pushname.
          const raw = p.senderName || p.pushname || p.notifyName || "";
          const clean = String(raw).trim().replace(/^[.\-_\s]+$/, "");
          senderName = (clean.length > 1 && !/^\d+$/.test(clean)) ? clean : "";
        }
        ...
        if (isGroup) {
          console.log(`📨 [GRUPO: ${groupName}] ${senderName||senderPhone}: ${text.slice(0, 60)}`);
          console.log(`   ↳ debug group fields: chatName="${p.chatName}" senderName="${p.senderName}" phone="${p.phone}" participantPhone="${p.participantPhone}"`);
        } else {
          console.log(`📨 ${new Date().toLocaleTimeString("pt-BR")} | ${senderName||senderPhone}: ${text.slice(0, 80)}`);
        }
