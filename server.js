...
const recentSends = []; // {text, ts} — tracks messages sent from platform to dedup fromMe webhooks
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      if (needsText) {
        ...
        if (fromMe && !isGroup) {
          console.log(`📱 fromMe phone extraction: chatId=${p.chatId||"?"} phone=${p.phone||"?"} to=${p.to||"?"} → extracted=${phone}`);
        }
        let senderName;
        if (fromMe) {
          senderName = "Você";
        ...
        // For fromMe direct chats: normalize phone to match existing conversations
        // (prevents ghost conversations from LID-format phone mismatches)
        let finalPhone = phone;
        if (fromMe && !isGroup) {
          // Try to match to an existing known phone
          const existing = webhookMessages.find(m => !m.isGroup && phoneMatch(m.phone, phone) && m.phone !== phone);
          if (existing) {
            finalPhone = existing.phone;
            console.log(`📱 fromMe phone: ${phone} → ${finalPhone}`);
          }
          // Dedup: if the TEXT matches a recently sent message from the platform, skip entirely.
          // This catches the Z-API echo webhook that comes back with a garbage phone number.
          const nowTs = Math.floor(Date.now() / 1000);
          const textTrimmed = (text || "").trim();
          const isDup = recentSends.some(s => s.text === textTrimmed && (nowTs - s.ts) < 120);
          if (isDup) {
            console.log(`🔄 Skip fromMe echo: "${textTrimmed.slice(0,40)}" (already injected from platform)`);
