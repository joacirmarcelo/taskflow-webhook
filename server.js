...
const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      webhookMessages.forEach(m => {
        if (!m.fromMe && !m.read && phoneMatch(m.phone, phone)) {
          m.read = true;
          count++;
          if (m.messageId) unreadIds.push(m.messageId);
        }
      });
      // Also tell WhatsApp to mark as read (so phone shows as read too)
      if (zapiInst && zapiTok && unreadIds.length > 0) {
        const clientToken = process.env.ZAPI_CLIENT_TOKEN || "";
        // Z-API read-message endpoint — mark each unread message as read on WhatsApp
        for (const msgId of unreadIds.slice(0, 10)) { // limit to 10 to avoid rate limits
          try {
        ...
        console.log(`👁 Marcou ${unreadIds.length} msg(s) como lida(s) no WhatsApp para ${phone}`);
      }
      return json(res, { ok: true, marked: count, syncedToWhatsApp: unreadIds.length });
  ...
  if (url === "/debug-raw" && req.method === "GET") {
    return json(res, { version: "2025-06-12-v4", lastRawWebhook, lidMapSize: Object.keys(lidToPhone).length, lidMap: lidToPhone });
  }
