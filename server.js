...
const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (needsText) {
        ...
        if (fromMe && !isGroup) {
          ...
          // Note: if LID wasn't resolved, finalPhone stays as the LID number.
          // The frontend handles this by not creating ghost conversations for non-BR phones.
        }
  ...
  if (url === "/debug-raw" && req.method === "GET") {
    return json(res, { version: "2025-06-11-v3", lastRawWebhook, lidMapSize: Object.keys(lidToPhone).length, lidMap: lidToPhone });
  }
