...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      if (needsText) {
        // Normalize phone — strip @c.us / @g.us / @lid AND the :XXXXXXXXXX LID suffix
        // Also truncate to 13 digits max (Brazil: 55 + 2 area + up to 9 mobile)
        ...
        const phone = phoneFull.length > 13 ? phoneFull.slice(0, 13) : phoneFull;
        ...
        if (fromMe) {
          ...
          senderName = clean.length > 1 ? clean : (phone || "Desconhecido");
        }
        ...
        if (!fromMe && !msgData.isGroup) {
          const cfg = autoReplyContacts.find(c => c.enabled && (
            c.phone === phone || phone.endsWith(c.phone.slice(-8)) || c.phone.endsWith(phone.slice(-8))
          ));
  ...
  if (url === "/webhook-messages/mark-read" && req.method === "POST") {
    ...
    try {
      ...
      let count = 0;
      webhookMessages.forEach(m => {
        const mp = String(m.phone || "").replace(/@.+$/, "").replace(/\D/g, "");
        if (!m.fromMe && !m.read && (mp === norm || mp.endsWith(norm.slice(-8)) || norm.endsWith(mp.slice(-8)))) {
          m.read = true;
          count++;
        }
      });
  ...
  if (url.startsWith("/auto-reply/sequence-state") && req.method === "GET") {
    ...
    return json(res, { state: sequenceState[phone] || { step: 0 } });
  }
