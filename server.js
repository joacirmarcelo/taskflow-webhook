...
// ── AUTO-REPLY ────────────────────────────────────────────────────────────────
const autoReplyContacts = []; // {phone, name, enabled, instructions, zapiInst, zapiTok, claudeKey}

async function generateAndSendAutoReply(config, messageText, senderName, phone) {
  const claudeKey = config.claudeKey || process.env.CLAUDE_API_KEY;
  if (!claudeKey || !config.zapiInst || !config.zapiTok) return;

  // Avoid replying to own messages
  if (!messageText || !messageText.trim()) return;

  try {
    // Last 6 messages from this contact for context
    const history = webhookMessages
      .filter(m => m.phone === phone)
      .slice(0, 6)
      .reverse()
      .map(m => `${m.fromMe ? "Assistente" : (m.senderName || senderName)}: ${m.text}`)
      .join("\n");
    ...
    const result = await httpsPost("api.anthropic.com", "/v1/messages",
      { "Content-Type": "application/json", "x-api-key": claudeKey, "anthropic-version": "2023-06-01" },
      {
        ...
        system: config.instructions ||
          "Você é assistente cordial da Unirad Centro de Saúde, Capão da Canoa/RS. Responda de forma breve, profissional e acolhedora em português. Não mencione que é uma IA.",
        messages: [{
          role: "user",
          content: `Histórico da conversa:\n${history}\n\nNova mensagem de ${senderName}: ${messageText}\n\nRedija uma resposta breve e adequada. Responda APENAS com o texto da mensagem, sem explicações.`
        }]
      }
    );
    ...
    await httpsPost("api.z-api.io",
      `/instances/${config.zapiInst}/token/${config.zapiTok}/send-text`,
    ...
    console.log(`🤖 Auto-reply → ${config.name || phone}: ${replyText.slice(0, 80)}`);
  } catch (e) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      if (text.trim()) {
        ...
        console.log(`📨 ${new Date().toLocaleTimeString("pt-BR")} | ${senderName}: ${text.slice(0, 80)}${audioTag}`);

        // ── Auto-reply check ────────────────────────────────────────────────
        if (!p.fromMe && !isGroup) {
          const config = autoReplyContacts.find(c => c.enabled && (
            c.phone === phone ||
            phone.endsWith(c.phone.slice(-8)) ||
            c.phone.endsWith(phone.slice(-8))
          ));
          if (config) {
            setTimeout(() => generateAndSendAutoReply(config, text, senderName, phone), 1500);
          }
        }
      }
    } catch (e) {
  ...
  // ── Auto-reply ────────────────────────────────────────────────────────────
  if (url === "/auto-reply" && req.method === "GET") {
    return json(res, autoReplyContacts);
  }
  if (url === "/auto-reply/save" && req.method === "POST") {
    const body = await readBody(req);
    try {
      const d = JSON.parse(body);
      const existing = autoReplyContacts.findIndex(c => c.phone === d.phone);
      const entry = {
      ...
      if (existing >= 0) autoReplyContacts[existing] = entry;
      else autoReplyContacts.push(entry);
      console.log(`⚡ Auto-reply ${entry.enabled ? "ativado" : "desativado"} para ${entry.name}`);
      return json(res, { ok: true });
  ...
  if (url === "/auto-reply/toggle" && req.method === "POST") {
    ...
    try {
      ...
      if (c) { c.enabled = enabled; return json(res, { ok: true, enabled: c.enabled }); }
  ...
  if (url === "/auto-reply/remove" && req.method === "POST") {
    ...
    try {
      ...
      const i = autoReplyContacts.findIndex(c => c.phone === phone);
      if (i >= 0) autoReplyContacts.splice(i, 1);
      return json(res, { ok: true });
    } catch (e) { return json(res, { error: e.message }, 400); }
  }
