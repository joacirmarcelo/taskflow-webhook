...
function learnLid(payload, realPhone) {
  ...
  [payload.chatId, payload.phone, payload.from].forEach(raw => {
    ...
    if (id && id !== realPhone && id.length >= 8) {
      // Check if it's a LID (doesn't look like a real Brazilian phone: no 55 prefix, or >13 digits)
      const looksLikeLid = !id.startsWith("55") || id.length > 13;
      if (looksLikeLid && lidToPhone[id] !== realPhone) {
        lidToPhone[id] = realPhone;
        changed = true;
        console.log(`🔗 LID mapped: ${id} → ${realPhone}`);
      }
    }
  });
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      if (needsText) {
        ...
        // LID mapping: learn from received messages, resolve for sent messages
        if (!fromMe && !isGroup && phone) {
          learnLid(p, phone); // Map any LID identifiers to this real phone
        }
        if (fromMe && !isGroup) {
        ...
        if (fromMe && !isGroup) {
          ...
          // 4. If STILL unresolved and doesn't look like a real BR phone, skip it
          // (prevents ghost conversations with random LID numbers)
          if (finalPhone === phone && !phone.startsWith("55")) {
            console.log(`⚠️ Skip unresolvable LID: ${phone} "${text.slice(0,30)}" (no mapping yet — need received msg first)`);
            return json(res, { ok: true });
          }
        }
