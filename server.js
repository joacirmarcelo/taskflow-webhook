    ...
    try {
      ...
      if (needsText) {
        ...
        if (fromMe && !isGroup) {
          ...
          // Dedup: skip if same text recently injected (<30s) for same contact
          const recent = webhookMessages.find(m => m.fromMe && m.text === text
            && phoneMatch(m.phone, phone) && (Date.now()/1000 - (m.timestamp||0)) < 30);
          if (recent) {
            console.log(`🔄 Skip dup fromMe: "${text.slice(0,40)}"`);
