...
function isVideoMessage(p) {
  return p.type === "video" || p.type === "gif" || p.type === "ptt-video" || !!p.video;
}

function extractVideoUrl(p) {
  return p.video?.videoUrl || p.video?.url || p.video?.fileUrl ||
         p.videoUrl || (typeof p.video === "string" ? p.video : null);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      // Handle read receipts — status "READ" means the RECIPIENT read YOUR sent message (blue ticks).
      // This does NOT mean you read incoming messages — don't mark received messages as read here.
      if (p.status === "READ" || p.status === "PLAYED" || p.type === "ReadCallback" || p.type === "MessageStatusCallback") {
        if (p.status === "READ" || p.status === "PLAYED") {
          // Update delivery status of sent messages (for blue ticks display)
          const ids = p.ids || (p.messageId ? [p.messageId] : []);
          ids.forEach(msgId => {
            const msg = webhookMessages.find(m => m.messageId === msgId && m.fromMe);
            if (msg) msg.readByRecipient = true;
          });
        }
        return json(res, { ok: true });
      }
      ...
      const isVideo = !isAudio && !isImage && isVideoMessage(p);
      const isDoc = !isAudio && !isImage && !isVideo && isDocumentMessage(p);
      ...
      if (isAudio) {
        ...
        msgType = "video";
        mediaUrl = extractVideoUrl(p);
        caption = p.caption || "";
        text = caption || "[Vídeo]";
        console.log(`🎬 Vídeo recebido: url=${mediaUrl?.slice(0,60)||"null"}`);
      } else if (isDoc) {
        msgType = "document";
      ...
      if (needsText) {
        ...
        if (fromMe && !isGroup) {
          // 0. Rebuild LID map from group messages in memory (survives Render deploys)
          if (Object.keys(lidToPhone).length === 0) {
            webhookMessages.forEach(wm => {
              if (wm.participantLid && wm.participantPhone) {
                const lid = String(wm.participantLid).replace(/@\S+$/, "").replace(/\D/g, "");
                const ph = String(wm.participantPhone).replace(/\D/g, "");
                if (lid && ph && ph.startsWith("55") && lidToPhone[lid] !== ph) {
                  lidToPhone[lid] = ph;
                }
              }
            });
            if (Object.keys(lidToPhone).length > 0) {
              console.log(`🔄 LID map rebuilt from memory: ${Object.keys(lidToPhone).length} entries`);
              saveLidMap();
            }
          }
          // 1. Try LID map
          ...
          // 3. Fallback: match by chatName (Z-API sends contact name for direct fromMe)
          if (finalPhone === phone && !isRealPhone(phone) && p.chatName) {
            const chatNameLower = (p.chatName || "").toLowerCase().trim();
            if (chatNameLower) {
              const match = webhookMessages.find(m => !m.fromMe && !m.isGroup && isRealPhone(m.phone)
                && (m.senderName || "").toLowerCase().trim() === chatNameLower);
              if (match) {
                finalPhone = match.phone;
                // Also learn this LID for future
                lidToPhone[phone] = finalPhone;
                saveLidMap();
                console.log(`📱 chatName match: "${p.chatName}" → ${finalPhone}`);
              }
            }
          }
        ...
        const msgData = {
          ...
          participantLid: p.participantLid || null,
          participantPhone: p.participantPhone || null,
          fromMe,
  ...
  // Test endpoint: verify Z-API modify-chat works
  if (url === "/test-read" && req.method === "POST") {
    const body = await readBody(req);
    try {
      ...
      // Test 1: modify-chat
      try {
        results.modifyChat = await httpsPost("api.z-api.io",
          `/instances/${zapiInst}/token/${zapiTok}/modify-chat`,
          { "Content-Type": "application/json", "Client-Token": clientToken },
          { phone, action: "read" });
      } catch (e) { results.modifyChat = { error: e.message }; }
      // Test 2: read-message (with latest messageId if available)
      const lastMsg = webhookMessages.find(m => !m.fromMe && m.messageId && phoneMatch(m.phone, phone));
      if (lastMsg) {
        try {
          results.readMessage = await httpsPost("api.z-api.io",
            `/instances/${zapiInst}/token/${zapiTok}/read-message`,
            { "Content-Type": "application/json", "Client-Token": clientToken },
            { phone, messageId: lastMsg.messageId });
        } catch (e) { results.readMessage = { error: e.message }; }
      }
      return json(res, { ok: true, phone, results });
  ...
  if (url === "/webhook-messages/mark-read" && req.method === "POST") {
    ...
    try {
      ...
      // Always try to mark as read on WhatsApp (even if count=0, the phone might have unread)
      let zapiResult = null;
      if (zapiInst && zapiTok) {
        const clientToken = process.env.ZAPI_CLIENT_TOKEN || "";
        try {
          zapiResult = await httpsPost("api.z-api.io",
            `/instances/${zapiInst}/token/${zapiTok}/modify-chat`,
            { "Content-Type": "application/json", "Client-Token": clientToken },
            { phone, action: "read" });
          console.log(`👁 mark-read ${phone}: platform=${count} zapi=${JSON.stringify(zapiResult).slice(0,60)}`);
        } catch (e) {
          console.log(`⚠️ mark-read failed for ${phone}: ${e.message}`);
          // Fallback: try read-message
          const lastMsg = webhookMessages.find(m => !m.fromMe && m.messageId && phoneMatch(m.phone, phone));
          if (lastMsg?.messageId) {
            try {
              zapiResult = await httpsPost("api.z-api.io",
                ...
                { "Content-Type": "application/json", "Client-Token": clientToken },
                { phone, messageId: lastMsg.messageId });
            } catch (e2) { zapiResult = { error: e2.message }; }
          }
        }
      }
      return json(res, { ok: true, marked: count, zapiResult });
    } catch(e) { return json(res, { error: e.message }, 400); }
  }
