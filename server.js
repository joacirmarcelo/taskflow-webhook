...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/webhook" && req.method === "POST") {
    ...
    try {
      ...
      if ((p.chatId || "").includes("@g.us")) {
        console.log("👥 GROUP payload keys:", Object.keys(p).join(", "));
        console.log("👥 chatName:", p.chatName, "| groupName:", p.groupName, "| senderName:", p.senderName, "| notifyName:", p.notifyName);
      }
      ...
      if (needsText) {
        ...
        if (isGroup) {
          ...
          // Z-API group name fields (varies by version). NEVER fall back to senderName
          // (that's the participant, not the group — caused groups to show person's name).
          groupName = p.chatName || p.groupName || p.group?.name || p.groupInfo?.subject
            || p.subject || p.notifyName || (groupId ? `Grupo ${groupId.slice(-6)}` : "Grupo");
        }
        ...
        console.log(`📨 ${new Date().toLocaleTimeString("pt-BR")} | ${isGroup ? "["+groupName+"] " : ""}${senderName}: ${text.slice(0, 80)}`);
  ...
  if (url.startsWith("/media-proxy") && req.method === "GET") {
    try {
      ...
      const ct = target.includes(".pdf") ? "application/pdf"
      ...
      res.writeHead(200, { "Content-Type": ct, "Content-Disposition": "inline" });
      res.end(buf);
      return;
    } catch (e) {
